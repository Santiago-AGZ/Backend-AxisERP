...
public class GetLowStockAlertsUseCaseImpl implements GetLowStockAlertsUseCase {
    ...
    private static final Logger log = LoggerFactory.getLogger(GetLowStockAlertsUseCaseImpl.class);
    ...
    public PageResult<ProductInventoryResponse> execute(int page, int size) {
        ...
        List<Inventory> inventories = inventoryRepositoryPort.findLowStock(page, size);
        long total = inventoryRepositoryPort.countLowStock();
        ...
        log.debug("low_stock_alerts page={} size={} total={}", page, size, total);
