...
public class ReactivateUserUseCaseImpl implements ReactivateUserUseCase {
    ...
    private static final Logger log = LoggerFactory.getLogger(ReactivateUserUseCaseImpl.class);
    ...
    @Override
    @Transactional
    public UserResponse reactivate(UUID id, UUID updatedBy) {
        User user = userRepositoryPort.findById(id)
                .orElseThrow(() -> new UserNotFoundException("Usuario no encontrado"));

        if (user.getStatus() == User.UserStatus.ELIMINADO) {
            throw new IllegalStateException("No se puede reactivar un usuario eliminado");
        }
        ...
        User reactivated = UserFactory.reactivate(user, updatedBy);
        User saved = userRepositoryPort.save(reactivated);
        ...
        auditService.log(AuditAction.REACTIVATE, "USER", saved.getId(),
                null, null,
                java.util.Map.of("previousStatus", "INACTIVO", "newStatus", "ACTIVO"),
                null, null);

        log.info("user_reactivated id={} email={}", saved.getId(), saved.getEmail());
