...
public class InvoiceController {
    ...
    public ResponseEntity<InvoiceResponse> getInvoice(@PathVariable UUID id) {
        return ResponseEntity.ok(getInvoiceUseCase.getById(id));
    }

    @PreAuthorize("hasAnyRole('ADMIN', 'VENDEDOR')")
    @GetMapping("/by-sale/{saleId}")
    public ResponseEntity<InvoiceResponse> getInvoiceBySale(@PathVariable UUID saleId) {
        return ResponseEntity.ok(getInvoiceUseCase.getBySaleId(saleId));
    }
}
